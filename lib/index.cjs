...
var yespowerWasm = require('yespower-wasm');
...
function randomNonce() {
  return BigInt(yespowerWasm.bytesToHex(crypto.webcrypto.getRandomValues(new Uint8Array(63))));
}
...
async function findNonce(work, reportOn = 100, workerId, algoPers, isWorker, msgSend) {
  const yespower = await yespowerWasm.Yespower.init();
  let timeStart = Date.now();
  let nonce = randomNonce();
  let i = 0;
  const blockTarget = work.blockTarget?.startsWith("0x") ? BigInt(work.blockTarget) : 0n;
  const target = BigInt(work.target);
  const header = yespowerWasm.hexToBytes(work.header);
  while (true) {
    const nonceBytes = setLength(yespowerWasm.hexToBytes(nonce.toString(16)), 8, false);
    const concatted = concatBytes(header, bytesReverse(nonceBytes));
    const result = BigInt(yespowerWasm.bytesToHex(yespower.Hash(concatted, void 0, void 0, algoPers)));
    const blockFound = blockTarget > result;
    if (target > result) {
      const found = {
        nonce: yespowerWasm.bytesToHex(nonceBytes),
        header: work.header,
...
class WebSocketPool extends eventemitter3.EventEmitter {
  url;
  login;
  worker;
  reconnect;
