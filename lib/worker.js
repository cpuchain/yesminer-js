...
function randomNonce() {
  return BigInt((0,lib.bytesToHex)(external_crypto_namespaceObject.webcrypto.getRandomValues(new Uint8Array(63))));
}
...
async function nodeWorker() {
  const work = (external_worker_threads_default()).workerData;
  const msgSend = (msg) => {
    external_worker_threads_default().parentPort?.postMessage(msg);
  };
